package com.studyMate.world.screens
{
	...
	public class WorldViewMediator extends ScreenBaseMediator
	{
		...
		public function WorldViewMediator(viewComponent:Object=null)
		{
...
//					sendNotification(WorldConst.SWITCH_SCREEN,[new SwitchScreenVO(BookshelfNewView2Mediator),new SwitchScreenVO(CleanGpuMediator)]);
		...
		{			
			...
			bubbles.dispose();
			Assets.disposeTexture("dayBg");
			Assets.disposeTexture("nightBg");
		...
		override public function handleNotification(notification:INotification):void
		{
			...
			switch(name)
			{
				...
				case WorldConst.UPDATE_WORLD_TIME:{
					
					nightImage.alpha = notification.getBody() as Number;
					
					if(nightImage.alpha>0.9){
						dayImage.removeFromParent();
					}else if(nightImage.alpha<0.1){
						nightImage.removeFromParent();
					}else if(!dayImage.parent){
						view.addChildAt(dayImage,0);
					}else if(!nightImage.parent){
						view.addChildAt(nightImage,1);
					}
