...
//			tf.htmlText = htmlStr.replace(reg, "<font size=\'18\'>　   </font>");//把文本框内的内容用<font>字体标签代替字符串htmlStr中的表情符号
			...
			while (kk< faceSign.length){
				if (pointObj[kk] != null){
					...
					obj.x = pointObj[kk].x+tf.x+3;//设置表情的x坐标
					obj.y = pointObj[kk].y+tf.y;//设置表情的y坐标
					obj = null;
				}
				kk++;
			}
		...
		//检查录音文件位置,在检查表情完了之后
		public function checRecord(htmlStr:String,left:Boolean = true):MovieClip{
//			var faceSign:Array = new Array();//表情符数组
//			var reg:RegExp = /\/:bq/g;  
//			faceSign = htmlStr.match(reg);//这里只搜索到两个表情符号
//			if (faceSign.length == 0)return null;
			if(left){				
				var faceClass:Class = AssetTool.getCurrentLibClass("sysRecordA");
			}else{
				faceClass = AssetTool.getCurrentLibClass("sysRecordB");
			}
	
			var obj:MovieClip = new faceClass();//faceMC为资源文件中的影片剪辑
			obj.name = "recordfaces";
			obj.gotoAndStop(0);//由于表情是以一个MC多帧来保存，所以计算出来的表情数，直接跳帧显示
			obj.url = htmlStr;
//			reg = null;			
			return obj;
		}
