package com.mylib.framework.model
{
	...
	import com.mylib.framework.model.vo.SendVO;
	...
	public class BackgroundWorkerThreadMediator extends Mediator implements IAnimatable
	{
		public static const NAME:String = "BackgroundWorkerThreadMediator";
...
//		private var background:BackgroundWorker;
		...
		private var afterSendVO:SendCommandVO;
		
		public function BackgroundWorkerThreadMediator(viewComponent:Object=null)
		{
		...
		override public function handleNotification(notification:INotification):void
		{
			...
			Facade.getInstance(CoreConst.CORE).sendNotification(CoreConst.FLOW_RECORD,new RecordVO("handleNotification"+notification.getName(),"BackgroundWorkerThreadMediator",0));
			...
			switch(notification.getName())
			{
				case CoreConst.SETUP_DATA_WORKER:
				{
					...
					workerLoader.addEventListener(IOErrorEvent.IO_ERROR,ioErrorHandle);
					
					var path:String;
					if(CONFIG::ARM){
						path = Global.document.resolvePath(Global.localPath+"module/BackgroundWorkerThread.swf").url
					}else{
						path = "BackgroundWorkerThread.swf";
					}
					workerLoader.load(new URLRequest(path));
				...
				case CoreConst.BG_SEND_COMPLETE:
					...
					if((arr[1] as DataResultVO).isBreak&&!quiting){
...
//							throw new Error("重复点击 default"+sendVO.receiveNotification+" "+(notification.getBody() as RemoteFileLoadVO).completeNotice);
			...
			Facade.getInstance(CoreConst.CORE).sendNotification(CoreConst.FLOW_RECORD,new RecordVO("handleNotification e","BackgroundWorkerThreadMediator",0));
		...
		private function sendCommand(data:Array):void{
			...
			if(bgWorkerCommandChannel){
				
				if(processing){
					cacheCommand.push(data);
					return;
				}
				
				getLogger(BackgroundWorkerThreadMediator).debug("sendCommand "+data[0]);
				bgWorkerCommandChannel.send(data);
			}
		...
		protected function ioErrorHandle(event:IOErrorEvent):void
		{
			...
			sendNotification(CoreConst.ERROR_REPORT,"BackgroundWorkerThread.swf miss!!!");
		...
		private function loadComplete(event:Event):void
		{
			...
			var workerBytes:ByteArray = workerLoader.data as ByteArray;
			worker = WorkerDomain.current.createWorker(workerBytes,true);
			
			
			Global.mainThread = Worker.current;
			
			uiMem = new ByteArray;
			uiMem.shareable = true;
			
			memLock = new Mutex();
			
			Global.mainThread.setSharedProperty(ShareConst.UI_MEM,uiMem);
			Global.mainThread.setSharedProperty(ShareConst.MEM_LOCK,memLock);
			...
			Worker.current.setSharedProperty("incomingCommandChannel", bgWorkerCommandChannel);
			worker.setSharedProperty("mainThread",Worker.current);
			...
			Worker.current.setSharedProperty("resultChannel", resultChannel);
			
			loadingChannel = worker.createMessageChannel(Worker.current);
			loadingChannel.addEventListener(Event.CHANNEL_MESSAGE, handleLoadingMessage);
			Worker.current.setSharedProperty("loadingChannel", loadingChannel);
		...
		private function handleLoadingMessage(event:Event):void{
			Global.mainThread.setSharedProperty(ShareConst.LOADING_DISABLE,true);
			while(loadingChannel.messageAvailable){
				var resultArr:Array = loadingChannel.receive() as Array;
				
				getLogger(BackgroundWorkerThreadMediator).debug(resultArr[0],resultArr[1]);
				
				sendNotification(resultArr[0],resultArr[1]);
			}
			Global.mainThread.setSharedProperty(ShareConst.LOADING_DISABLE,false);
		}
		...
		private function handleResultMessage(event:Event):void
		{
			...
			getLogger(BackgroundWorkerThreadMediator).debug("start process");
			processing = true;
			getLogger(BackgroundWorkerThreadMediator).debug("rec ");
			processResult();
			
			processing = false;
			getLogger(BackgroundWorkerThreadMediator).debug("end process");
		...
		private function processResult():void{
			...
			while(uiMem.bytesAvailable){
				var resultArr:Array = uiMem.readObject() as Array;
				getLogger(BackgroundWorkerThreadMediator).debug(resultArr[0]);
				sendNotification(resultArr[0],resultArr[1]);
			}
			memLock.lock();
			if(!uiMem.bytesAvailable){
				uiMem.clear();
			}
			memLock.unlock();
		...
		override public function listNotificationInterests():Array
		{
			return [CoreConst.SETUP_DATA_WORKER,CoreConst.BACKGROUND_COMMAND,CoreConst.SEND_11,CoreConst.SEND_1N,
				...
				CoreConst.DETECT_DATA,CoreConst.CONTINUE_SEND_REQUEST,
				CoreConst.SOUND_PLAY,CoreConst.SOUND_STOP,CoreConst.BG_RESEND
			];
		}
		
		private function detectData(event:Event):void{
			
			
			if(!processing&&uiMem&&uiMem.bytesAvailable){
				handleResultMessage(null);
			}
		...
		override public function onRegister():void
		{
			...
			Global.stage.addEventListener(Event.ENTER_FRAME,detectData);
		...
		{
			if(uiMem.bytesAvailable&&!processing){
				handleResultMessage(null);
			}
		}
