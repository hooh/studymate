package com.mylib.game.charater.logic.ai
{
	...
	public class HeroCollectAI implements IFightDecision
	{
		...
		public function HeroCollectAI(_fightData:HeroFightVO,myHero:GameCharater)
		{
		...
		public function makeFightDecision(ai:FighterControllerMediator):void
		{
			...
			ai.charater.action("speak",8,ai.fighter.attackRate*20,false);
			
			TweenLite.to(ai,ai.fighter.attackRate-0.2,{onComplete:attackComplete,onCompleteParams:[ai,reduceHP,isCrit]});
...
//			TweenLite.to(hpField,0.5,{y:hpField.y-60,alpha:0,delay:0.8,onComplete:hpFieldAimationHandle,onCompleteParams:[hpField]});
		...
		{
			if((fightData.cacheHP>fightData.averageDamage*3||fightData.remainHP<fightData.averageDamage*3)){
				
				if(Math.random() > 0.5)
					ai.go(Math.random()*60-10+fightData.monster.fighter.charater.view.x,ai.charater.view.y);
				else
